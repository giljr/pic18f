/* Project: Unit 06 - CCP
      Code: unit_06_02_capture16th.c

   Objective:   This capture will take 16 sampling mode.
   ...
   if(PIR1.CCP1IF == 1)
    {
        if(_state == T1)
        {
          ...
          t1 = ONE.INTEGER/16;         // t1: first sampling
         ...
         else if(_state == T2)
         {
          ...
          ONE.BYTE[0] = CCPR1L;
          ONE.BYTE[1]= CCPR1H;
          t2 = ONE.INTEGER/16;         // T2: second sampling
          _state = T1;                 // jump to the previews state T1
                                       // To write to LCD, turn interruption off!
          PIE1.CCP1IE = 0;
         }
...
#endif
 TRISB =0;
 TRISC.RC2 = 1;                        // Input sygnal goes here!
 PORTC.RC2 = 1;
}
...
{                                      // ccp for capture
  CCP1CON = 0;                         // Resetting the Module: Microchip recommendation
